...
# ---- Split P+T pairs in either train or test set ----
characteristics <- read_tsv(file='../TCGA/gdc_sample_sheet.2020-07-07.tsv')
recurrent <- read.delim("set3_recurrent_samples.txt") 
recurrent <- as.character(recurrent$recurrent.10.13.)
...
write(colnames(test_set),"test_set_samples3.txt")
...
for (i in 1:3) {
  ...
  for(i in 1:10) { 
    ...
    #Write results
    write(t(c(R2_train,R2_val)), 'performance3.txt', append=TRUE)
    write.table(genes, 'boruta3.txt', append=TRUE)
    write.table(var_imp$importance, 'varimp3.txt', append=TRUE)
    write(nrow(genes),"number_of_genes3.txt",append=TRUE)
  }
}

# ---- Combine results  ----
#Performance
performance <- read.delim("performance3.txt",header = FALSE, sep=" ")
...
#Boruta
boruta <- read.delim("boruta3.txt") %>% separate(col = genes, into = c("number", "gene"), sep = " ") %>% drop_na() %>% dplyr::select(-c(1)) %>% dplyr::group_by(gene) %>% summarise(count = n()) 
number_of_genes <- read.delim("number_of_genes3.txt",header=FALSE)
...
#Varimp
varimp <- read.delim("varimp3.txt") %>% separate(col = Overall, into = c("gene", "score"), sep = " ") %>% drop_na() 
