...
recurrent <- read.delim("set2_recurrent_samples.txt") 
recurrent <- as.character(recurrent$recurrent.5.9.)

train_rec <- recurrent[1:3]
train_rec <- characteristics %>% dplyr::filter(`Case ID`%in%train_rec) %>% pull(`Sample ID`)
train_rec <- counts[,train_rec]

test_rec <- recurrent[4:5]
test_rec <- characteristics %>% dplyr::filter(`Case ID`%in%test_rec) %>% pull(`Sample ID`)
test_rec <- test_rec[!test_rec%in%"TCGA-06-0211-01B"]
...
test_set <- dplyr::bind_cols(test_rec, test_set)

write(colnames(test_set),"test_set_samples2.txt")
...
for (i in 1:3) {
  ...
  for(i in 1:10) { 
    ...
    #Write results
    write(t(c(R2_train,R2_val)), 'performance2.txt', append=TRUE)
    write.table(genes, 'boruta2.txt', append=TRUE)
    write.table(var_imp$importance, 'varimp2.txt', append=TRUE)
    write(nrow(genes),"number_of_genes2.txt",append=TRUE)
  }
}

# ---- Combine results  ----
#Performance
performance <- read.delim("performance2.txt",header = FALSE, sep=" ")
...
#Boruta
boruta <- read.delim("boruta2.txt") %>% separate(col = genes, into = c("number", "gene"), sep = " ") %>% drop_na() %>% dplyr::select(-c(1)) %>% dplyr::group_by(gene) %>% summarise(count = n()) 
number_of_genes <- read.delim("number_of_genes2.txt",header=FALSE)
...
#Varimp
varimp <- read.delim("varimp2.txt") %>% separate(col = Overall, into = c("gene", "score"), sep = " ") %>% drop_na() 
